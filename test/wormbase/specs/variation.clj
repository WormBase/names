...
(s/def :variation/status (stc/spec {:spec string?
...
(s/def ::summary (stc/spec {:spec (s/merge ::wsp/provenance
                                           (s/keys :req-un [:variation/id :variation/status]
                                                   :opt-un [:variation/name]))
                            :description "A summary of the information held for a given variation."}))

(s/def ::status-changed (stc/spec {:spec (s/keys :req-un [:variation/status])}))

(s/def ::new (stc/spec {:spec (s/keys :req-un [:variation/name])
...
(s/def ::update-batch (stc/spec
                       {:spec (s/coll-of
                               (s/merge ::update (s/keys :req-un [:variation/id]))
                               :min-count 1)

...
(defn- gen-from-rand-name-pattern [kw]
...
(s/def :gene/cgc-name
  (s/spec (s/and st/string? not-empty)
          :gen (partial gen-from-rand-name-pattern :gene/cgc-name)))

;; TODO add proper spec 
(s/def :gene/sequence-name st/string?)
